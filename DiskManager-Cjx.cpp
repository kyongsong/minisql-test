...
page_id_t DiskManager::AllocatePage() {
  ...
  //4.Get the Logical_Page_id(Extent_id*(BIT_MAP_SIZE+1)+NextFreePage_id)
  ...
  uint32_t NextPage=0;
  //Situation 0: if the this is the first Allocations
  ...
  //Situation 1: General Case.
      ...
      ReadBitMapPage(meta_page->num_extents_-1,Page_Data);
      ...
      if(state==true)
      {
          page_id_t BitMap_page_id=(meta_page->num_extents_-1)*(BITMAP_SIZE+1)+1;  
        //Write Back BitMap Page  to the Disk
        char *Page_Data = reinterpret_cast<char *>(Bitmap_page);
          WritePhysicalPage(BitMap_page_id,Page_Data);
          NextPage=(meta_page->num_extents_-1)*BITMAP_SIZE+NextPage;
      }
      else
      {
...
void DiskManager::DeAllocatePage(page_id_t logical_page_id) {
  ...
  //2.if we DeAllocatePage, We may need to update _num_extents_
  //3.The We need to read bitmap in that Extent from the disk
  ...
  {
      std::cerr<<"Situation 1.2-----Can not Deallocate,Page "<<logical_page_id<<" is Free"<<endl;
  }
  //Situation 2: General Case.
      ...
      {
        std::cout<<"Situation 2.1-----disk_manager::DeAllocate-----Failed"<<std::endl;
      }
    }
